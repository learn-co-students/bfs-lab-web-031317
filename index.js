const bfs = (rootNode, vertices, edges) => {
    ...
    while (queue.length !== 0) {
        ...
        markDistanceAndPredecessor(node, adj).forEach( marked => queue.push(marked) )
    }
    return vertices
}

const findAdjacent = (address, vertices, edges) => {
    let adjacent = []
    edges.forEach( edge => {
...
const markDistanceAndPredecessor = (pred, adjacentNodes) => {
